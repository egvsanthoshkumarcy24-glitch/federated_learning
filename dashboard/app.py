...
if nav_choice == "Admin Dashboard":
    ...
    active = sum([1 for c in COMPANIES if load_config(c).get("dataset_path")])
    with col1:
        st.markdown(f"<div class='glass-container'><div class='metric-label'>Active Companies</div><div class='metric-value'>{active} / 5</div></div>", unsafe_allow_html=True)
        
    global_model_exists = os.path.exists("models/weights/Global_Model_v1.weights.h5")
    with col2:
        st.markdown(f"<div class='glass-container'><div class='metric-label'>Global Base Model</div><div class='metric-value'>{'Available' if global_model_exists else 'Missing'}</div></div>", unsafe_allow_html=True)
    ...
    with fl_col1:
        ...
        if st.button("Trigger Federated Round", type="primary"):
            if active < 2:
                ...
                st.success("Federated Learning process launched. Check terminal for Flower logs.")

    with fl_col2:
        if fl_rounds > 0:
            st.subheader("Global Convergence (F1 Score)")
            rounds = list(range(1, fl_rounds + 1))
            f1s = [x["f1"] for x in fl_log]
            fig, ax = plt.subplots(figsize=(10, 4))
            fig.patch.set_facecolor('#0d1117')
            ax.set_facecolor('#0d1117')
            ax.plot(rounds, f1s, marker='o', color='#58a6ff', linewidth=2)
...
# ======================================================================
    ...
    with tab_upload:
        ...
        if selected_file:
            ...
            ts_col = st.selectbox("Timestamp Column (used for ordering only)", cols, 
                                  index=cols.index(cfg["timestamp_col"]) if "timestamp_col" in cfg and cfg["timestamp_col"] in cols else 0)
